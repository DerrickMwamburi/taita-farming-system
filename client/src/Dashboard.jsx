  ...
  const fetchDatabase = async () => {
    ...
    try {
      const [farmersRes, cropsRes, alertsRes, ticketsRes, backupsRes, adminsRes, meRes] = await Promise.all([
        ...
        fetch('http://127.0.0.1:8000/api/tickets/', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('http://127.0.0.1:8000/api/backups/', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('http://127.0.0.1:8000/api/admins/', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('http://127.0.0.1:8000/api/admins/me/', { headers: { 'Authorization': `Bearer ${token}` } }) // <--- FETCH CURRENT USER
      ]);
