...
function PublicForm() {
  const [formData, setFormData] = useState({ full_name: '', phone_number: '', subcounty: '', acreage:'', crops: [] });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ full_name: '', phone_number: '', subcounty: '', acreage: '', crops: [] });
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-md p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
            <label className="block text-sm font-semibold text-gray-700">Farm Size (Acres)</label>
            <input
              type="number"
              step="0.1"
              min="0.1"
              required
              className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 bg-gray-50"
              placeholder="e.g. 2.5"
